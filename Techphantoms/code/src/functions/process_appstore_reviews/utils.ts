...
export class ApiUtils {
  ...
  async queryAppStoreReviews(appId: string, country: string, page: number, rapidapiKey: string) {
    const url = `https://app-store12.p.rapidapi.com/apps/${appId}/reviews?country=${country}&page=${page}&sort=recent`;
    const options = {
      method: 'GET',
      headers: {
        'X-RapidAPI-Key': rapidapiKey,
